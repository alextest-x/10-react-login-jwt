...
export const useUsers = () => {
    ...
    //para validar en los botones para que solo el admin lo muestre
    //y en usuario no 
    ...
    const handlerAddUser =async (user) => {
      if (!login.isAdmin) return; //sino es admin se sale 
      let  response;
         ...
         //valida busca por el constrain del campo usernam y email
                   ...
                   if(error.response.data?.message?.includes('UK_username')) {
                    ...
                    if(error.response.data?.message?.includes('UK_email')) {
                      ...
                      handlerLogout(); //cerrar la sesion cuando expira
